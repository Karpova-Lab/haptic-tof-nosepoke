# This code is adapted from Adafruit's VL53L4CD library: https://docs.circuitpython.org/projects/vl53l4cd/en/latest/ - Andy Lustig May 2024
...
from pyControl.hardware import Analog_input, Digital_output
...
class VL53L4CD(Analog_input):
    def __init__(
        ...
        self.LED.off()
        self.motor = Digital_output(port.POW_B)
        self.motor.off()
        self.tof_enable = Digital_output(port.DIO_C)
        self.tof_enable.off()

        self.buffer = bytearray(2)
        self.get_range_packet = struct.pack(">H", 0x0096)
        self.clear_packet = struct.pack(">H", 0x0086) + b"\x01"
        self.proximity = 0
        self.entrance = 0
        self._ranging = False
        self.connection_error = False

    def i2c_setup(self, new_address=None):
        self.tof_enable.on()
        self._sensor_init()
        if new_address:
    ...
    def read_sample(self):
        try:
            self.i2c.writeto(self._address, self.clear_packet)  # clear interrupt
            self.i2c.writeto(self._address, self.get_range_packet)
            self.i2c.readfrom_into(self._address, self.buffer)
            self.proximity = int.from_bytes(self.buffer, "big")

            return self.entrance - self.proximity
        except:
    ...
    def _write_register(self, address, data, length=None):
        if length is None:
            length = len(data)
        self.i2c.writeto(self._address, struct.pack(">H", address) + data[:length])

    def _read_register(self, address, length=1):
        data = bytearray(length)
        self.i2c.writeto(self._address, struct.pack(">H", address))
        self.i2c.readfrom_into(self._address, data)
        return data
